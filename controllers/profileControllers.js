...
const updateProfilePic = async (req, res) => {
  try {
    ...
    const oldProfilePic =
      stytchUser?.trusted_metadata?.profilePic ?? null;

    if (profilePic === null) {
      await db.query(
        `UPDATE users SET profile_pic = NULL WHERE stytch_user_id = $1`,
        [stytchUserId]
      );
      ...
      await db.query(
        `UPDATE users SET profile_pic = $1 WHERE stytch_user_id = $2`,
